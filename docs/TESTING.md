...
| 2026-09-02 | 9 — Meta | **117 passed / 0 failed** (exit 0, BOTH runner paths) — adds 9 meta tests: XP curve boundaries, save round-trip + checksum, corruption fallback (invalid JSON / wrong checksum / newer version → defaults + quarantine, never a crash), wallet + spend refusal, high-score cap/sort/best, skin rules (level gate, price, equip-owned-only, SnakeController zero-skin-code scan), missions (3/day deterministic, max-per-run + cumulative semantics, reward payout, reroll-once gate), leaderboard backend (submit/fetch/order/meta, new-best detection), settings→save migration | PASS (mock AND null) | PASS — game-over now shows COINS count-up; menus carry MISSIONS + SKINS | UI harness 3/3 landscape + portrait PASS post-Phase 9; **restart survival live-proven**: fresh process probe `CC_RESTART_PROBE coins=83 best=1390 level=2 scores=3 missions=3` after a 3-run harness session (save.json checksummed, wallet/XP/high-scores/missions all reload); live save inspected (83 coins = payouts incl. top-3 bonus, XP 417 = Σscore/10, 3 high scores top 1390) |
| 2026-09-02 | 10 — Feel | **117 passed / 0 failed** (exit 0; suite unchanged — feel work is render/audio side, covered by harnesses below) | PASS (mock AND null) | PASS ×3 landscape + portrait (confetti path exercised on fresh-install first game-over) | **Draw calls measured for the first time: 1073 → 91 max (budget 150, decision #71)** via new harness metrics `draw_calls_max` + `CC_DC_DIAG`; **hex boundary lattice pixel-verified** (28 bright runs ~4 px, `CC_BOUND_SHOT` hook); **collected types render in colour** (close-up probe: all 5 types; in-game HUD frame: cyan 218 px + green 88 + orange 82); 20 procedural SFX + 3 seamless music loops generated and wired (dummy driver — human-verified); autoplay unlock gate; engine facts logged: MultiMesh stride 12 without colors / 16 padded with color_array |
| 2026-09-02 | 11 — Web + monetization | **123 passed / 0 failed** (exit 0, BOTH runners) — adds 6 monetization tests: WebPortal no-JS degradation (immediate DISABLED + show_completed on every path — the contract whose absence hung PAUSED_FOR_AD, caught live in-browser), AdMob scaffold degradation, AdMob-type-leak scan, consent flow (accept/decline persist + consent.cfg reload + DECLINE blocks serving with reason consent-denied), remote-config seam (apply/refuse/keep/clear), analytics session counters | PASS (mock AND null) | PASS — native UI harness now drives the CONSENT screen first (`CC_UI_CONSENT_ACCEPTED`); **IN-BROWSER (Chromium 151, SwiftShader WebGL2, playwright driver `tools/webverify.js`)**: `CC_SMOKE_OK` with AdProviderWebPortal(mock); **full UI harness `CC_UI_VERIFY_PASS` in-browser** incl. mock-portal rewarded REVIVE (ad_requested→shown→reward_granted→SHOWN_COMPLETED) + INTER_RUN + MENU_RETURN + touch steering; `cc_ad_provider=null` build `CC_SMOKE_OK` in-browser. Build: raw 38.8 MB / **gz transfer 10.9 MB (< 25 MB budget)** |
